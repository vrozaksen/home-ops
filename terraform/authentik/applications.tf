...
# ─────────────────────────────────────────────────────────────────────────────────
# OIDC credentials — generated by Terraform, synced to Infisical /terraform/authentik/oidc
# ─────────────────────────────────────────────────────────────────────────────────
...
locals {
  ...
  parsed_secrets = {
    for app in local.oidc_apps : app => {
      client_id     = random_string.oidc_client_id[app].result
      client_secret = random_password.oidc_client_secret[app].result
    }
  }
}

resource "random_string" "oidc_client_id" {
  for_each = toset(local.oidc_apps)
  length   = 40
  special  = false
}
...
resource "infisical_secret" "oidc_client_id" {
  for_each     = toset(local.oidc_apps)
  name         = "${upper(replace(each.key, "-", "_"))}_CLIENT_ID"
  value        = random_string.oidc_client_id[each.key].result
  env_slug     = "prod"
...
resource "infisical_secret" "oidc_client_secret" {
  for_each     = toset(local.oidc_apps)
  name         = "${upper(replace(each.key, "-", "_"))}_CLIENT_SECRET"
  value        = random_password.oidc_client_secret[each.key].result
  env_slug     = "prod"
  workspace_id = "da94b011-9a7d-408b-92d9-55be47efe750"
  folder_path  = "/terraform/authentik/oidc"
}
