...
class AnimesubinfoProvider(Provider):
    ...
    def _pick_archive_member(subtitle, subtitle_files):
        ...
        home-ops patch. Packs on animesub.info hold a whole season, so taking
        the first member gives every episode the subtitles of episode 1.
        Deliberately avoids `os` — upstream dropped that import in 1.6.1.
        """
        ...
        for name in subtitle_files:
            ...
            if any(n in numbers for n in wanted):
                logger.debug(f'Archive member {name!r} matches episode {wanted[0]}')
                return name
